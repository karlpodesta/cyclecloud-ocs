...
from ocs import autoscaler, environment, util, validate
from ocs.driver import GridEngineDriver, HostgroupConstraint
from ocs.environment import from_qconf
...
def create_support_archive(config: Dict, archive: str) -> None:
    ...
    def _add_contents(contents: str, name: str) -> None:
        tarinfo = tarfile.TarInfo("ocs-support/" + name)
...
def complexes(config: Dict, include_irrelevant: bool = False) -> None:
    ...
    if include_irrelevant:
        ge_config = config.get("ocs", {})
        if "relevant_complexes" in ge_config:
            ge_config.pop("relevant_complexes")

    relevant = set(config.get("ocs", {}).get("relevant_complexes", []))
...
def initconfig(writer: TextIO = sys.stdout, **config: Dict) -> None:
    ...
    #   :logging => {:config_file => "#{node[:cyclecloud][:bootstrap]}/ocs/logging.conf"},
    #   :default_resources => [ { :select => {}, :name => "slots", :value => "node.vcpu_count"} ],
    #   :ocs => {:hostgroups
    #                   :relevant_complexes =>relevant_complexes,
    #                   :idle_timeout => node[:ocs][:idle_timeout]}
    # }
    if "ocs" not in config:
        config["ocs"] = {}

    for pe_name in config.pop("disable_pgs_for_pe", []):
        config["ocs"]["pes"] = pes = config["ocs"].get("pes") or {}
    ...
    for expr in config.pop("hostgroup_constraints", []):
        ...
        if "hostgroups" not in config["ocs"]:
            config["ocs"]["hostgroups"] = {}
        if hg_name not in config["ocs"]["hostgroups"]:
            config["ocs"]["hostgroups"][hg_name] = {}
        config["ocs"]["hostgroups"][hg_name]["constraints"] = constraint_json

    for expr_json in config.pop("default_hostgroups", []):

        if "default_hostgroups" not in config["ocs"]:
            config["ocs"]["default_hostgroups"] = []
        if set(list(expr_json.keys())) != set(["select", "hostgroups"]):
            ...
            sys.exit(1)
        config["ocs"]["default_hostgroups"].append(expr_json)
...
def main(argv: Iterable[str] = None) -> None:
    default_install_dir = os.path.join("/", "opt", "cycle", "ocs")
    ...
    support_archive_parser.add_argument(
        "--archive", "-a", default="ocs_support-{}.tar.gz".format(time.time())
    )
