...
from ocs import environment as envlib
from ocs import validate as validatelib
from ocs.allocation_rules import FixedProcesses
from ocs.complex import Complex
from ocs.environment import GridEngineEnvironment
from ocs.hostgroup import BoundHostgroup, QuotaConstraint
from ocs.parallel_environments import ParallelEnvironment
from ocs.qbin import check_output
from ocs.queue import GridEngineQueue
from ocs.util import add_node_to_hostgroup, get_node_hostgroups
        ...
        for hostgroup in self.ge_env.qbin.qconf(["-shgrpl"]).split():
            ...
            for sched_hostname in scheduler_hostnames:
                if sched_hostname not in hostnames:
                    ...
                    logging.warning(
                        'To disable this behavior, set {"ocs": "add_schedulers_to_hostgroups": false}}'
                    )
                    hg_path = None
                    try:
                        ...
                        logging.getLogger("ocs.driver").info(
                            "hostgroup contents written to %s", hg_path
                        )
                        logging.getLogger("ocs.driver").info(contents)
...
affinity    0.000000"""
    ...
    def handle_post_delete(self, nodes_to_delete: List[Node]) -> None:
        ...
        logging.getLogger("ocs.driver").info("handle_post_delete")
    ...
    def handle_join_cluster(self, matched_nodes: List[Node]) -> List[Node]:
        ...
        logging.getLogger("ocs.driver").info("handle_join_cluster")
    ...
    def add_nodes_to_cluster(self, nodes: List[Node]) -> List[Node]:
        ...
        logging.getLogger("ocs.driver").info("add_nodes_to_cluster")
    ...
    def _add_node_to_cluster(
        ...
        if not hostgroups:
            logging.warning(
                ...
                + "'ocs_hostgroups = @preferred-hostgroup' to the nodearray's configuration "
                ...
                + '{"ocs": {"default_hostgroups": [{"select": {}, "hostgroups": ["@myhostgroup"]}]}}',
                node,
            )
    ...
    def get_host_template(self, node: Node) -> str:
        ge_config = self.autoscale_config.get("ocs", {})
...
license_oversubscription NONE"""
    ...
    def add_exec_host(self, node: Node) -> None:
        ...
        try:
            ...
            logging.getLogger("ocs.driver").info(host_template)
    ...
    def clean_hosts(self, invalid_nodes: List[SchedulerNode]) -> List[Node]:
        logging.getLogger("ocs.driver").info("clean_hosts")
    ...
    def preprocess_config(self, config: Dict[str, Any]) -> Dict[str, Any]:
        """
            ...
            config["ocs"] = ge_config = config.get("ocs", {})
        ...
        if not config.get("ocs", {}).get("relevant_complexes"):
            return config

        if "ccnodeid" not in config["ocs"]["relevant_complexes"]:
            config["ocs"]["relevant_complexes"].append("ccnodeid")
...
def _get_jobs_and_nodes(
    ...
    relevant_complexes = (
        autoscale_config.get("ocs", {}).get("relevant_complexes") or []
    )
...
def _parse_scheduler_nodes(
    ...
    for qiqle in sorted(elems, key=keyfunc):
        ...
        # TODO RDH
        compute_node.metadata["ocs_hostgroups"] = ",".join(
...
def _parse_job(
    ...
    if len(requested_queues) != 1 or ("*" in requested_queues[0]):
        logging.error(
            ...
            + ' for autoscale, change {"ocs": {"queues": {"%s": {"autoscale_enabled": false}}}}'
...
def _is_node_disabled(node: Node) -> bool:
    # ocs uses single letter state codes
