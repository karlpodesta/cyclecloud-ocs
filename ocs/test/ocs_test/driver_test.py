...
def test_custom_parser() -> None:
    ...
    node = SchedulerNode("tux", {"_ocs_hostgroups": q.hostgroups_sorted},)
    ...
    assert q.satisfied_by_node(node)
    assert q.do_decrement(node)
    assert node.available["_ocs_hostgroups"] == q.hostgroups_sorted
    assert node.software_configuration["ocs_hostgroups"] == " ".join(
        q.hostgroups_sorted
    )
...
def test_preprocess_configs() -> None:
    ...
    assert {
        ...
        "ocs": {"relevant_complexes": ["slots", "ccnodeid"]},
    } == d.preprocess_config(
        {
            "default_resources": [{"name": "slots", "select": {}, "value": 1}],
            "ocs": {"relevant_complexes": ["slots"]},
        }
    )
