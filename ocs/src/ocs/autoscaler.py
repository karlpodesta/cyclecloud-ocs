...
from ocs.util import get_node_hostgroups

if typing.TYPE_CHECKING:
    from ocs.driver import GridEngineDriver
...
def autoscale_grid_engine(
    ...
    if dry_run:
        logging.warning("Running ocs autoscaler in dry run mode")
...
def new_demand_calculator(
    ...
    if node_history is None:
        ...
        if not db_path:
            db_dir = "/opt/cycle/jetpack/system/bootstrap/ocs"
...
def new_driver(config: Dict, ge_env: GridEngineEnvironment) -> "GridEngineDriver":
    ...
    # # just shorthand for ocs.deferdriver.DeferredDriver
    # if ge_config.get("driver_scripts_dir"):
    #     deferred_qname = "ocs.deferdriver.DeferredDriver"
    #     if ge_config.get("driver", deferred_qname) == deferred_qname:
    #         ge_config["driver"] = deferred_qname

    driver_expr = ge_config.get("driver", "ocs.driver.new_driver")
...
class BadDriverError(RuntimeError):
    ...
    def __str__(self) -> str:
        return (
            "Expected ocs.driver=module.func_name"
            + " or ocs.driver=module.class_name. Got {}".format(self.bad_expr)
        )
