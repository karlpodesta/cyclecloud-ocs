...
if typing.TYPE_CHECKING:
    from ocs.hostgroup import BoundHostgroup, Hostgroup  # noqa: F401
...
def get_node_hostgroups(config: Dict, node: Node) -> List[str]:
    hostgroups_expr = node.metadata.get("ocs_hostgroups")

    if not hostgroups_expr:
        hostgroups_expr = node.software_configuration.get("ocs_hostgroups")

    if not hostgroups_expr:
        default_hostgroups = config.get("ocs", {}).get("default_hostgroups", [])
        for dh in default_hostgroups:
            if "select" not in dh:
                logging.warning(
                    "Missing key 'select' in ocs.default_hostgroups %s", dh
                )
                continue
            if "hostgroups" not in dh:
                logging.warning(
                    "Missing key 'hostgroups' in ocs.default_hostgroups %s", dh
                )
                continue
            ...
            if satisfied:
                ...
                node.metadata["ocs_hostgroups"] = hostgroups_expr
...
def add_node_to_hostgroup(
    ...
    for attr in [_metadata] if node.exists else [_metadata, _node_override]:
        d = attr(node)
        hostgroups_expr = d.get("ocs_hostgroups")
        if not hostgroups_expr:
            d["ocs_hostgroups"] = hostgroup.name
        elif hostgroup.name not in hostgroups_expr:
            d["ocs_hostgroups"] = "{} {}".format(hostgroups_expr, hostgroup.name)
