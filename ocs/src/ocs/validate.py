    ...
    for pe in queue.get_pes("*"):
        if pe.requires_placement_groups:
            ...
            if "@allhosts" in pe_hostgroups:
                ...
                warn(
                    '             {"ocs": {"pes": {"%s": {"requires_placement_groups": false}}}}',
                    pe.name,
                )
    return not failure
...
def validate_nodes(config: Dict, dcalc: DemandCalculator, warn: WarnFunction) -> bool:
    ...
    for node in dcalc.node_mgr.get_nodes():
        ...
        if not hostgroups:
            failure = True
            warn(
                "%s does not define ocs_hostgroups and will not be able to join the cluster.",
                node,
            )
...
def validate_default_hostgroups(
    ...
    for dh in default_hostgroups:
        ...
        if set(list(dh.keys())) != set(["select", "hostgroups"]):
            warn(
                "    Invalid entry in ocs.default_hostgroups. Expected select and hostgroups as keys. %s",
                dh,
            )
            failure = True
            continue
        ...
        for hg in hostgroups:
            if hg not in ge_env.hostgroups:
                warn(
                    "    Unknown hostgroup %s. See ocs.default_hostgroups - %s",
                    hg,
                    dh,
                )
                failure = True
