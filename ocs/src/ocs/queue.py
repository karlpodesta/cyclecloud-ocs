...
from ocs.complex import parse_queue_complex_values
from ocs.hostgroup import BoundHostgroup, Hostgroup
from ocs.qbin import QBin
from ocs.util import parse_ge_config, parse_hostgroup_mapping

if typing.TYPE_CHECKING:
    from ocs.complex import Complex  # noqa: F401
    from ocs.environment import GridEngineEnvironment
    from ocs.parallel_environments import ParallelEnvironment  # noqa: F401
    from ocs.scheduler import GridEngineScheduler
        ...
        for hostgroup, pes_for_hg in pe_list.items():
            for pe_name in pes_for_hg:
                ...
                if pe_name not in pes:
                    logging.warning(
                        'Unknown parallel environment %s defined in {"ocs": {"pes": {"%s": {}}}} - %s. Skipping',
                        pe_name,
                        pe_name,
                        list(pes.keys()),
                    )
...
def read_queues(
    ...
    autoscale_queues_config = autoscale_config.get("ocs", {}).get("queues", {})
