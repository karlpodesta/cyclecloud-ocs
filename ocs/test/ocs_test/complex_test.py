...
from ocs.complex import Complex, parse_queue_complex_values, read_complexes
from ocs.hostgroup import make_quota_bound_consumable_constraint, process_quotas
from ocs.qbin import QBinImpl
from ocs.util import json_dump
from ocs_test.autoscaler_test import common_ge_env
...
"""
    ...
    complexes = read_complexes({"ocs": {"relevant_complexes": []}}, qbin)
    complexes = read_complexes({}, qbin)
    assert len(complexes) == 143  # hand calculated

    complexes = read_complexes({"ocs": {"relevant_complexes": None}}, qbin)
    complexes = read_complexes({}, qbin)
    assert len(complexes) == 143  # hand calculated

    complexes = read_complexes({"ocs": {"relevant_complexes": ["slots"]}}, qbin)
    assert len(complexes) == 2
    assert set(complexes.keys()) == set(["slots", "s"])

    complexes = read_complexes(
        {"ocs": {"relevant_complexes": ["h", "mfree"]}}, qbin
    )
